...
def get_review(repo_files_data: list[RepoFileModel], assignment_description : str, candidate_level: RepoFileType) -> str:
    code_data_to_send = [convert_from_repo_file_to_file_for_review(x) for x in repo_files_data] # convert to reduce size
    code_data_to_send_json = [x.model_dump_json(exclude_none=True) for x in code_data_to_send] # exclude none for same reason
